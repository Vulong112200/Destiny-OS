...
**18** methodology đã được đăng ký vào registry với trạng thái thật (đối chiếu `docs/RESEARCH_BLOCKERS.md`): **5** `PRODUCTION_READY` (Tarot, Thần số học Pythagoras, Lịch Việt Nam truyền thống, Phong Thủy Bát Trạch, và Kinh Dịch — hào làm chủ), nhiều mục `CONTENT_REQUIRED` tính được (`BAZI_TUBINH_CHART` — lập lá số Tứ Trụ **và Đại Vận**; các lớp quẻ từ/hào từ của Kinh Dịch; góc chiếu lá số chiêm tinh), còn lại `RESEARCH_REQUIRED`/`DECISION_REQUIRED`/`OUT_OF_SCOPE`. Phase 8 chiếm **hai** entry chứ không phải một: gộp lại thì buộc phải nói sai theo hướng này (che một lá số đang chạy được sau `RESEARCH_REQUIRED`) hoặc hướng kia (hàm ý đã có Dụng Thần).
